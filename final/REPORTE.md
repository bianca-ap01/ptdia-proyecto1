...
Como contraste externo, el [estudio TEI de Forrester encargado por Mastercard](https://tei.forrester.com/go/mastercard/decisionintelligenceandrulesservices/) modela 0.75 minutos de analista por alerta a USD 52/h, es decir, **USD 0.65 por revisión**, y USD 4 de trabajo posterior para un *subconjunto* de falsas alertas. El [estudio 2025 de LexisNexis](https://risk.lexisnexis.com/insights-resources/research/US-CA-true-cost-of-fraud-study) informa **USD 5.75 de costo total por cada USD 1 de pérdida directa** para servicios financieros estadounidenses. Son referencias de organizaciones y mercados distintos, no tarifas observadas en IEEE-CIS: no se conoce la divisa de `TransactionAmt` ni qué parte de ese multiplicador ya incluye trabajo operativo. La fórmula propuesta, `0.65 × revisiones + 4 × escalaciones + 5.75 × monto de falsos negativos`, es por ello una **hipótesis de sensibilidad**, no un costo validado. El término de USD 4 corresponde a seguimiento de falsas alertas, no necesariamente a toda escalación; si revisar o escalar falla, también existe fraude residual.

[`cost_sensitivity.py`](cost_sensitivity.py) recalcula tres especificaciones sobre las predicciones guardadas y **optimiza umbrales y estrategia sólo en validación**, con el cupo diario derivado del entrenamiento. El [CSV de resultados](cost_sensitivity_valid.csv) conserva los 27 candidatos por escenario. Sin costo de actualización, los resultados principales son:

| Escenario de validación | LightGBM estático V01 | Mejor candidato | Costo ilustrativo por transacción del mejor |
|:--|--:|:--|--:|
| Fórmula literal: USD 4 por toda escalación y prevención perfecta | 8.025 | LightGBM periódico, 45 días | 7.783 |
| USD 4 sólo por escalación legítima; eficacia 80 %/95 % | 9.942 | LightGBM periódico, 45 días | 9.722 |
| USD 4 por toda escalación; eficacia 60 %/95 % | 11.147 | LightGBM periódico, 45 días | 10.937 |

El primer porcentaje de eficacia corresponde a revisión y el segundo a escalamiento. La selección de 45 días es estable en estas tres hipótesis, pero **no se recalculó el holdout** bajo ellas: ese periodo ya influyó en decisiones históricas y no sería una prueba final nueva. Tampoco se interpretan estas cifras como ahorro monetario real, dado el posible solapamiento del multiplicador 5.75 con los costos operativos.
...
La recomendación posterior de mantener V01 se tomó **después de observar esa reversión en holdout**. Es una decisión exploratoria de operación; el costo de V01 en ese mismo holdout no puede presentarse como estimación final independiente de una política recién seleccionada. Para esa afirmación haría falta un periodo futuro adicional. Las decisiones históricas V01–V04 también consultaron métricas de holdout, y la auditoría V03 usó distribuciones sin etiqueta de holdout al recomendar variables; esos resultados documentan exploración, no una selección con prueba futura intacta.
