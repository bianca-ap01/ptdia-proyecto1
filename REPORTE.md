...
Se excluyen `TransactionID`, `TransactionDT` absoluto y sus índices de día/semana de los 185 predictores de V01. `TransactionDT` sí ordena eventos, determina cortes y forma la hora de la transacción. Los cuartiles del indicador de monto atípico y los mapas/frecuencias categóricos se ajustan exclusivamente con la ventana histórica de cada actualización. El indicador de monto atípico de V01 se construyó con cuartiles calculados sobre todas las filas en lugar de su ventana histórica. El efecto está cuantificado: [`leakage_impact.py`](leakage_impact.py) entrena el mismo modelo dos veces —idéntico en semilla, hiperparámetros y las 185 variables— cambiando solo el origen de los cuartiles. Los cortes se desplazan de `(−80.13, 248.08)` con ajuste histórico a `(−79.20, 247.52)` con todas las filas, un **0.22 %** en el corte superior, y eso altera el valor de la variable en **6 de las 88 581 filas de holdout** (0.007 %). La PR-AUC de holdout resulta **0.51656 con fuga y 0.51910 sin ella**: la versión contaminada rinde **0.0025 peor**, no mejor. La fuga se declara porque existe, pero no otorga ventaja detectable: la comparación entre V01 y las estrategias adaptativas no está sesgada por esta vía. El [detalle](kaggle/experimentacion/outputs/leakage_impact.csv) queda registrado. El UID se forma con atributos de la propia operación, y su condición conocido/desconocido usa solo transacciones anteriores al bloque. El origen y momento de disponibilidad de las columnas anonimizadas `V`, `C`, `D` e `id` no están documentados a nivel suficiente para garantizar que todas estén libres de fuga. En producción se exigiría contrato de disponibilidad al instante de la decisión y una auditoría de linaje. Véase la [auditoría por campo](kaggle/eda/outputs/leakage_audit.csv).

El enunciado pide atributos que incorporen la dimensión temporal: ventanas deslizantes, variables rezagadas o agregaciones por periodo. Se evaluaron cinco variables con memoria por entidad, todas construidas con `shift(1)` dentro del UID ordenado por tiempo, de modo que una fila nunca ve su propio valor ni el futuro: número de operaciones previas de la entidad, monto medio histórico, desvío frente a esa media, segundos desde la operación anterior y monto rezagado. [`temporal_features.py`](temporal_features.py) las evalúa.

El modelo **sí las usa**: quedan en las posiciones 19, 21, 26, 29 y 43 de 190 por importancia. Pero el aporte es marginal: PR-AUC de holdout **0.51910 → 0.51972** (**+0.0006**), con una caída simétrica en validación (0.55009 → 0.54882). La lectura razonable es que las columnas anonimizadas `C*` y `D*` ya codifican recurrencia por entidad —los `D*` son explícitamente contadores de días desde eventos previos—, de modo que el lag explícito reexpresa señal que el modelo ya tenía. Se documenta el experimento y no se incorporan al pipeline: añadirían cinco variables y un `groupby` costoso por cada reajuste a cambio de una ganancia dentro del ruido. El [detalle](kaggle/experimentacion/outputs/temporal_features_impact.csv) queda registrado.
...
El desempeño por tamaño de ventana sigue un patrón cóncavo idéntico en las tres familias: 14 días olvida demasiado, el historial acumulado arrastra observaciones antiguas, y el óptimo se ubica en el rango de **30 a 45 días**. Se adopta 45 días por tener la media más alta en las tres familias, con la salvedad —cuantificada abajo— de que dentro de ese rango la diferencia no es distinguible del ruido. LightGBM periódico de 45 días supera al V01 estático en **0.046 puntos de PR-AUC media** y requiere **cinco actualizaciones** en holdout; la versión por alerta de 45 días obtiene 0.574 con **dos**.

Con cinco bloques y una sola semilla, una diferencia de medias no basta para afirmar un ranking. Como las estrategias comparten los mismos bloques, la prueba adecuada es un **t pareado**, que elimina la varianza entre bloques —la que domina, con desviaciones de 0.05 a 0.07 dentro de cada configuración. El [análisis de significancia](kaggle/experimentacion/outputs/significance_summary.csv), reproducible con [`significance_analysis.py`](significance_analysis.py), delimita el alcance de cada afirmación:
...
El experimento sustenta dos afirmaciones de alcance distinto. **Adaptar supera a no adaptar en las tres familias**, con intervalos que excluyen el cero y las cinco diferencias por bloque positivas. Dentro del rango óptimo, en cambio, **45 días no es distinguible de 30 días** en LightGBM ni en XGBoost: esa diferencia cabe dentro del ruido, de modo que se recomienda 45 días por su media pero sin atribuirle superioridad estadística sobre 30. Lo demostrado es el valor de la ventana deslizante frente al modelo congelado, no el ajuste fino de su tamaño. Esto responde qué cambia al variar la memoria y cuánto se entrena. Las curvas [LightGBM](kaggle/experimentacion/outputs/plots/01_pr_auc_lightgbm.png), [XGBoost](kaggle/experimentacion/outputs/plots/01_pr_auc_xgboost.png) y [CatBoost](kaggle/experimentacion/outputs/plots/01_pr_auc_catboost.png) permiten ver los cinco bloques, no solo su promedio.

![PR-AUC de LightGBM por bloque futuro y estrategia](kaggle/experimentacion/outputs/plots/01_pr_auc_lightgbm.png)

El trabajo usa **dos contabilidades de costo que no son intercambiables**, y conviene distinguirlas porque ordenan las configuraciones de forma distinta. El **proxy de ranking** de esta sección aplica el cupo como un corte fijo del 5 % superior del score, sin cola diaria ni fricción por escalamiento; sirve para comparar capacidad de ordenamiento. La **política completa** de la sección 5 impone el cupo día por día, separa revisar de escalar y cobra la fricción de cada legítima retenida; es la que modela la operación.

Bajo el proxy de ranking, el mejor costo lo obtiene LightGBM periódico con historial (**14.09**), seguido de V01 estático (**14.34**) y del periódico de 45 días (**14.47**). Bajo la política completa el orden se invierte y V01 pasa al primer lugar. Que el orden cambie entre ambas no es una inconsistencia: es el argumento central del trabajo, porque muestra que **la métrica que se optimiza determina el modelo que se elige**. Las decisiones operativas de este informe se toman con la política completa; el proxy se reporta solo para comparar discriminación. La política diaria y sus costos de actualización se evalúan aparte en el notebook 4. Las referencias tradicionales obtuvieron PR-AUC de holdout **0.170** (regresión logística) y **0.381** (árbol de decisión); se interpretan como comparadores breves con codificación numérica heredada, no como búsqueda exhaustiva de hiperparámetros. Los árboles nuevos usan tratamiento nativo de nulos; el imputador de las dos referencias tradicionales se ajustó solo con train inicial.
...
La selección previa por costo en **validación** favoreció LightGBM periódico de 45 días: **7.690 USD ilustrativos por transacción antes del cargo de actualización**, o **7.697** al sumar cuatro actualizaciones supuestas de USD 100, frente a **7.858** de V01, según [umbrales y costo de validación](kaggle/sistema_final/outputs/policy_thresholds_valid.csv) y [registro de ajustes](kaggle/experimentacion/outputs/fit_log.csv). Los umbrales se congelan ahí y se aplican a holdout sin retocar, porque reoptimizar sobre el periodo reservado sería sobreajuste de selección. Bajo esa disciplina, la configuración seleccionada en validación **no sostiene su ventaja de costo** frente a V01 en holdout: la tabla siguiente presenta esa medición. La clasificación por costo de todas las configuraciones es análisis posterior, no una nueva selección de hiperparámetros.
...
Esa comparación es el resultado principal del trabajo. Mover la eficacia del analista de 60 a 100 % desplaza el costo en **1.824 USD por transacción**; mover el cargo por reentrenar de USD 0 a 500 lo desplaza **0.028**. Es una diferencia de **65×** entre las dos palancas. Bajo esta función de costo, **el sistema está limitado por la capacidad de revisión humana, no por la calidad del clasificador**: ganar 0.046 de PR-AUC no se convierte en ahorro porque, con el cupo fijo en 158 revisiones diarias, un modelo que ordena mejor no puede revisar más y empuja masa hacia el escalamiento, que cuesta USD 10 por legítima.

De ahí la recomendación operativa, que no es «no adaptar». La adaptación mejora la discriminación con significancia estadística en las tres familias; lo que no se traduce, bajo los supuestos de costo declarados, es en ahorro. Por eso se mantiene V01 con política y se conserva armada la infraestructura adaptativa —ventanas, detector, protocolo de promoción—, con el disparo automático de reentrenamiento suspendido hasta que un piloto mida la eficacia real del analista, el parámetro que domina la decisión. El aparato de medición que permite detectar esa divergencia entre métrica y costo es el entregable; el modelo es secundario.
...
**Despliegue verificado.** El modelo se sirve como API en Cloud Run: [`serving/`](serving/README.md) contiene la aplicación FastAPI, el `Dockerfile` y el `cloudbuild.yaml` que construye, publica en Artifact Registry y despliega en un solo paso. El servicio expone `/predict` —score, probabilidad calibrada y acción de la política juntas— más `/health` y `/metrics`. Está desplegado y respondiendo en `https://fraude-api-agz3gnkzgq-uc.a.run.app`.

Eso permite reemplazar una estimación por una medición. El cómputo interno por solicitud es de **2.8 ms**, mientras que la latencia de extremo a extremo desde Lima hacia `us-central1`, sobre doce solicitudes secuenciales, tiene **mediana de 404 ms** (rango 364–617). Es decir, **cerca del 99 % del tiempo de respuesta es red y no modelo**: optimizar el clasificador no mejoraría la experiencia, y desplegar en una región cercana sí. Conviene contrastar esto con la cifra de inferencia por lote de la sección anterior, que mide otra cosa: allí el modelo procesa miles de filas juntas, aquí responde una a la vez con todo el costo de la solicitud HTTP.
