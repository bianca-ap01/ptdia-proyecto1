...
La línea base V01 usa LightGBM con 185 atributos y \emph{early stopping}. La selección se justifica por la naturaleza tabular del dataset: variables numéricas, categóricas codificadas, faltantes e interacciones no lineales. Posteriormente se ejecutaron controles: V01b con todas las features, V02 con features UID históricas, V03 de auditoría de variables, V04 con LightGBM, XGBoost y CatBoost sobre sets auditados, y V07 con baseline tradicional y MLP. EDA y V01--V07 quedan trazables como kernels públicos de Kaggle.

\begin{table}[H]\centering\small
\begin{tabularx}{\linewidth}{@{}p{.12\linewidth}X p{.23\linewidth}p{.22\linewidth}@{}}\toprule
...
V04 & Comparar LightGBM/XGBoost/CatBoost & Ninguno supera V01 & Mantener V01.\\
V05 & Traducir score en acción & Costo holdout 1.3680 & Política inicial.\\
V06 & Robustez y calibración & Brier 0.0479 a 0.0219 & Calibrar probabilidades.\\
V07 & Comparar MLP y baseline lineal & MLP PR-AUC 0.2218 & Benchmark, no reemplazo.\\\bottomrule
\end{tabularx}
\end{table}

Cada versión responde a una pregunta concreta. V01 mide el desempeño base con un algoritmo adecuado para datos tabulares. V01b prueba si aumentar la superficie de variables mejora la generalización; no lo hace. V02 mide si la memoria de cliente aporta señal; aporta en UID conocido, pero no en UID desconocido, por lo que sería riesgoso adoptarla como regla única. V03 separa variables para conservar, monitorear o revisar. V04 contrasta alternativas de boosting. V05 evalúa la parte de decisión, V06 la incertidumbre y V07 la comparación con red neuronal y baseline tradicional. Esta secuencia documenta el proceso pedido por la rúbrica, no sólo el resultado final.
...
La caída de PR-AUC entre validación y holdout es consistente con degradación temporal. V02 demuestra que el historial UID sí contiene señal: en holdout UID conocido, \texttt{uid\_freq\_amt} sube de 0.5946 a 0.6831; sin embargo, UID desconocido cae de 0.5458 a 0.5315. Por ello no se adopta como modelo único. V04 confirma que podar agresivamente features reduce señal: el mejor candidato auditado queda en 0.5131 de PR-AUC holdout, por debajo de V01. V07 agrega un MLP ponderado y una regresión logística SGD: en holdout obtienen PR-AUC 0.2218 y 0.1638, por debajo de LightGBM de referencia con 0.5313.

\textbf{Nota sobre la rúbrica.} V07 agrega un baseline tradicional lineal y un MLP. Si el docente exige literalmente dos modelos tradicionales distintos, aún conviene agregar un bosque aleatorio simple; esto no cambiaría la recomendación actual, pero cerraría ese ítem formal.
...
En V05, los costos normalizados son 100 por fraude aprobado, 10 por legítima escalada y 2 por revisión manual. La política se selecciona en validación y se aplica a holdout sin recalibrar. La corrida pública de Kaggle confirma los mismos umbrales y métricas que el script local.
...
V05 supera a reglas simples, aunque la revisión en holdout (5.10\%) excede levemente el objetivo de 5\%. En producción, si el cupo fuera estricto, se recomienda usar umbrales por percentil temporal o una cola con capacidad fija. V06, también ejecutado como kernel público, confirma que la política responde de forma coherente ante escenarios alternativos de costo y capacidad.

\section{Cobertura de la rúbrica}

\begin{table}[H]\centering\small
\begin{tabularx}{\linewidth}{@{}p{.28\linewidth}X@{}}\toprule
Criterio de evaluación & Evidencia incluida\\\midrule
Problema, objetivos, restricciones y métricas & Caso de fraude financiero, objetivos técnicos y operativos, PR-AUC, costo, revisión, escalamiento y calibración.\\
Análisis y preparación de datos & EDA temporal, tasa semanal, faltantes, UID, adversarial validation, split 70/15/15 y control de leakage temporal.\\
Modelado y evaluación & V01--V07 ejecutados en Kaggle, comparación temporal, holdout futuro y segmentación UID.\\
Sistema adaptativo & Arquitectura con predicción, decisión, acción, monitoreo, calibración y reentrenamiento por ventanas.\\
Calidad del producto & Repositorio GitHub, kernels públicos, reportes por versión, informe técnico, presentación e infografía.\\
Presentación & Deck ampliado para 15--20 minutos, con decisiones y fundamentos por etapa.\\\bottomrule
\end{tabularx}
\end{table}

Esta cobertura evita una entrega centrada sólo en el modelo. La guía del curso exige explicar cómo el sistema adquiere datos, predice, decide, actúa, mide incertidumbre y se adapta ante cambios. Por eso la solución final se presenta como un ciclo de vida completo.
